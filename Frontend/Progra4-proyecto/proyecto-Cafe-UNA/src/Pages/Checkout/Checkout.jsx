...
import { ArrowLeft, Coffee, FileText, ShoppingCart, Store, UploadCloud } from 'lucide-react';
...
import { registrarCompra, solicitarProforma } from '../../services/comprasService';
...
const Checkout = () => {
  ...
  const tGenerarProforma = useTraducir('Generar proforma');
  const tGenerandoProforma = useTraducir('Generando proforma...');
  const tProformaHint = useTraducir('Generá un PDF informativo con los totales calculados por el servidor. No realiza una compra ni reserva stock.');
  ...
  const handleGenerateProforma = async () => {
    if (cartItems.length === 0 || generatingProforma) {
      return;
    }

    if (!ubicacionCodigo) {
      setProformaError('Seleccioná el punto de venta para generar la proforma.');
      return;
    }

    const punto = puntosConDisponibilidad.find((item) => item.code === ubicacionCodigo);
    if (!punto?.cubrePedido) {
      setProformaError('El punto de venta seleccionado no tiene stock suficiente para generar la proforma.');
      return;
    }
    ...
    try {
      ...
      const usuario = getCurrentUser();
      const pdf = await solicitarProforma({
        clienteNombre: usuario?.name || usuario?.username || usuario?.email || 'Cliente',
        clienteCorreo: usuario?.email || usuario?.correo || '',
        ubicacionCodigo,
        ubicacionId: punto.id,
      ...
      link.href = url;
      link.download = 'proforma-cafe-una.pdf';
      ...
      setProformaError(error?.message || 'No se pudo generar la proforma.');
    } finally {
    ...
    <main className="checkout-page">
      ...
      <aside className="checkout-shell__pay">
        ...
        <p className="checkout-pay__lead">{tDetallesCompra}</p>
            {paymentError ? <p className="checkout-page__error" role="alert"><ST>{paymentError}</ST></p> : null}
            {proformaError ? <p className="checkout-page__error" role="alert"><ST>{proformaError}</ST></p> : null}
            ...
            <div className="checkout-page__actions">
              <button
                className="checkout-page__proforma"
                type="button"
                onClick={handleGenerateProforma}
                disabled={generatingProforma}
              >
                <FileText size={18} strokeWidth={2.3} aria-hidden="true" className="checkout-page__button-icon" />
                <span>{generatingProforma ? tGenerandoProforma : tGenerarProforma}</span>
              </button>
              <p className="checkout-page__proforma-hint">{tProformaHint}</p>
